...
/**
 * Immutable parameter object containing context and state information for agent
 * subscriber callbacks.
 * <p>
 * This record encapsulates all the essential information that subscriber methods need to
 * understand the current state of an agent execution. It provides access to the
 * conversation history, current agent state, the executing agent instance, and the
 ...
 *
 * @param messages the current list of messages in the conversation history, including
 * both user and agent messages up to this point
 * @param state the current internal state of the agent, containing execution context and
 * any intermediate data
 * @param agent the agent instance that is currently executing and generating these
 * subscriber callbacks
