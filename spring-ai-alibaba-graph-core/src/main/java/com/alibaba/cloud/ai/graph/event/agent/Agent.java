 ...
 * execution, including message processing, tool calls, and state management.
 * <p>
 * Agent execution is designed to be non-blocking through the use of CompletableFuture,
 * allowing for efficient resource utilization and responsive user interfaces. Progress
...
public interface Agent {
	 ...
	 * immediately with a CompletableFuture that will complete when the agent run finishes
	 * (either successfully or with an error).
	 * <p>
	 * During execution, the agent will invoke methods on the provided subscriber to
	 * communicate progress, events, and state changes in real-time. This allows for
	 * monitoring, logging, and user interface updates during long-running agent
	 * operations.
	 * <p>
	 ...
	 * @param subscriber the callback interface for receiving real-time updates about
	 * agent execution progress, events, and state changes
	 * @return a CompletableFuture that completes when the agent run finishes, with null
