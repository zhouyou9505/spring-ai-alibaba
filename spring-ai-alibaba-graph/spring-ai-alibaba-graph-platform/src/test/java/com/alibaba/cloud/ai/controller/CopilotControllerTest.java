...
class CopilotControllerTest {
    ...
    void testAdjustWorkflow_CreateNewSystem_Success() throws Exception {
        ...
        request.put("userRequest", "创建一个智能客服系统，包含问题分类Agent(问题分类包含分类tool)、数学的交给数据Agent，翻译的交给翻译Agent，数据分析的交给数据分析Agent(用web_search工具)，数据分析最后生成报告的Agent(llm类型)。");
