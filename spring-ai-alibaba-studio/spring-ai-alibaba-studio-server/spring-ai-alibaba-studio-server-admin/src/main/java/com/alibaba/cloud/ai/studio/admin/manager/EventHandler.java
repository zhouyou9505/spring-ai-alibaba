 ...
 *
 * 按照 AGUI 标准，支持所有事件类型的实时推送： - 生命周期事件：RUN_STARTED, RUN_FINISHED, RUN_ERROR, STEP_STARTED,
...
public class EventHandler {
	 ...
	 */
	public void registerSession(String runId, SseEmitter emitter) {
		sessionEmitters.put(runId, emitter);
		log.info("Registered SSE emitter for session: {}", runId);

		// 设置完成和超时回调
		emitter.onCompletion(() -> {
			sessionEmitters.remove(runId);
			log.info("SSE emitter completed for session: {}", runId);
		});

		emitter.onTimeout(() -> {
			sessionEmitters.remove(runId);
			log.info("SSE emitter timeout for session: {}", runId);
		});

		emitter.onError((ex) -> {
			sessionEmitters.remove(runId);
			log.error("SSE emitter error for session: {}", runId, ex);
		});
	}
