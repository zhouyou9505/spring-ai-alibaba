 ...
 *
 * 按照 AGUI 标准，处理所有事件类型： - 生命周期事件：RUN_STARTED, RUN_FINISHED, RUN_ERROR, STEP_STARTED,
 * STEP_FINISHED - 文本消息事件：TEXT_MESSAGE_START, TEXT_MESSAGE_CONTENT, TEXT_MESSAGE_END -
 * 工具调用事件：TOOL_CALL_START, TOOL_CALL_ARGS, TOOL_CALL_END, TOOL_CALL_RESULT -
 * 状态管理事件：STATE_SNAPSHOT, STATE_DELTA, MESSAGES_SNAPSHOT - 特殊事件：RAW, CUSTOM
 */
@Slf4j
public class CallbackManagerImpl implements CallbackManager {
	...
	public CallbackManagerImpl(EventHandler eventManager) {
	...
	public void onTextMessageContentEvent(TextMessageContentEvent event) {
		try {
			log.debug("Sending TEXT_MESSAGE_CONTENT event: messageId={}, delta={}", event.getMessageId(),
					event.getDelta());
			eventManager.emitEvent(event);
		}
		catch (Exception e) {
	...
	public void onTextMessageChunkEvent(TextMessageChunkEvent event) {
		try {
			log.debug("Sending TEXT_MESSAGE_CHUNK event: messageId={}, chunk={}", event.getMessageId(),
					event.getChunk());
			eventManager.emitEvent(event);
		}
		catch (Exception e) {
	...
	public void onThinkingTextMessageStartEvent(ThinkingTextMessageStartEvent event) {
		try {
			log.debug("Sending THINKING_TEXT_MESSAGE_START event: messageId={}, role={}", event.getMessageId(),
					event.getRole());
			eventManager.emitEvent(event);
		}
		catch (Exception e) {
	...
	public void onThinkingTextMessageContentEvent(ThinkingTextMessageContentEvent event) {
		try {
			log.debug("Sending THINKING_TEXT_MESSAGE_CONTENT event: messageId={}, delta={}", event.getMessageId(),
					event.getDelta());
			eventManager.emitEvent(event);
		}
		catch (Exception e) {
	...
	public void onToolCallStartEvent(ToolCallStartEvent event) {
		try {
			log.debug("Sending TOOL_CALL_START event: toolCallId={}, toolCallName={}", event.getToolCallId(),
	...
	public void onToolCallResultEvent(ToolCallResultEvent event) {
		try {
			log.debug("Sending TOOL_CALL_RESULT event: messageId={}, toolCallId={}", event.getMessageId(),
					event.getToolCallId());
			eventManager.emitEvent(event);
		}
		catch (Exception e) {
	...
	public void onToolCallChunkEvent(ToolCallChunkEvent event) {
		try {
			log.debug("Sending TOOL_CALL_CHUNK event: toolCallId={}, chunk={}", event.getToolCallId(),
					event.getChunk());
			eventManager.emitEvent(event);
		}
		catch (Exception e) {
